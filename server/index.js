...
mercadopago.configure({
  access_token:
    "TEST-1240325420739939-020922-42448ebbd0e851b677f29160a363d094-810596829", //TEST-4374591154479135-020922-e20e72c0b154cb839ca3f5c5398c21d5-1677617166 - mercado de pago
});
...
app.post("/crear-guitarra", async (req, res) => {
  const { nombre, precio, imagen, color } = req.body;
...
app.put("/actualizar-guitarra", async (req, res) => {
  ...
  try {
    const actualizacionGuitarra = await Guitarra.findByIdAndUpdate(
      id,
      { nombre, precio },
      { new: true }
    );
